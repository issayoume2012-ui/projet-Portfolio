import streamlit as st

st.title("Issa Youme")

st.header("Technicien Supérieur en Agronomie | Spécialisé en Production Animale et Végétale", width=10754)
st.write(" Compétent en traitement de sol avec la pédologie, en développement de la productivité des plantes et animales, en communication rural, en gestion administrative et économique des entreprises agricoles")
with st.sidebar:
    st.image("issa1.jpg")
    st.write("Leona, Saint-Louis")
    st.write("issayoume2012@gmail.com")
        
    st.subheader("Disponible pour un entretien. Vous pouvez me joindre facilement par Mail")
st.header("Formations")
st.write("👉🏿2024-2027 : Licence en Agronomie, Alimentaire et Nutritionelle, Université Amadou Makhtar Mbow de Dakar")
st.write("👉🏿2023-2024 : Bachelier Scientifique en Serie S2, Lycée d'Excellence Privé Aimé Césaire, Saint-Louis")


st.header("Compétences Techniques")
st.write("👉🏿Capable d'analyser et de suivre les récoltes avec Biomathématiques, les nouvelles technologies, la télédétection, la cartographie")
st.write("👉🏿Capable d'étudier un sol et à le rémédier avec la pédologie agricole")
st.write("👉🏿Connais le fonctionnement des plantes et des animaux avec la physiologie Animale et végétale, la botanique, la zoologie, la reproduction, nutrition")
st.write("👉🏿Capable d'améliorer les cultures avec les Nouvelles Technologies : biotechnologie, bio-informatique, l'entomologie, l'irrigation, la fertilisation")
st.write("👉🏿Compétent en sociologie rurale et en gestion d'entreprise")


st.header("Compétences Communicationnelles")
st.write("👉🏿Capable de dialoguer et de trouver des solutions consensuelles avec les acteurs agricoles")
st.write("👉🏿Capable d'écrire un rapport, de préparer et de présenter des documents sous format PowerPoint en français et en angalais")
 
st.header("Compétences Personnelles")
st.write("👉🏿Capacité à travailler en Groupe et à s'adapter vite")
st.write("👉🏿Capacité à communiquer avec les différents acteurs")
st.write("👉🏿Capable de travailler sous pression")
st.write("👉🏿Leadership")
st.write("👉🏿Ponctualité")
st.write("👉🏿Autonomie")
st.header("Compétence Numérique")
st.write("👉🏿Capable d'utiliser et d'exploiter Word, Excel,PowerPoint, Canva avec un Niveau Approfondie")
st.write("👉🏿Capable d'utiliser et d'exploiter Python, AlgoBox, Jupiterlab avec niveau débutant")





